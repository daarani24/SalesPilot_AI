...
_catalog_store_instance = None
 
def get_catalog_store() -> "CatalogVectorStore":
    """
    Returns the shared CatalogVectorStore, creating it on first call.
    Safe to call from multiple places -- only builds the index once.
    """
    global _catalog_store_instance
    if _catalog_store_instance is None:
        _catalog_store_instance = CatalogVectorStore()
    return _catalog_store_instance
 
if __name__ == "__main__":
   
    test_query = "laptop under 40000 for a college student"
    print(f"\nTest query: {test_query}\n")
    for i, chunk in enumerate(get_catalog_store().search(test_query), 1):
        print(f"--- Result {i} ---\n{chunk}\n")
